...
- 数学は中1の教科書進度に沿って14単元（正負の数・加法と減法・乗法と除法・数の集合と素数の積・
  ...
  生成結果のみをコミットしている）
- 英語も同じ方針で中1範囲を15単元（I am/You are・This is/He is・日時曜日天気・一般動詞・
  冠詞と複数形・代名詞・三単現のs・命令文と感嘆文・疑問詞・現在進行形・can・前置詞・
  過去形（一般動詞）・過去形（be動詞）と過去進行形・未来表現）に分割し、各単元30問を収録。
  単語・動詞活用のデータバンクから機械的に文を組み立てるため、活用形の誤りや主語と動詞の
  不一致が入り込まない（`lib/problems/english-grade1.ts` に生成結果のみをコミット）。
  さらに各単元には演習前に読む導入レッスン文（`lib/lessons/`）を用意し、単元ページの先頭に表示する
- 国語・理科・社会も中1範囲を各4単元に分割し、1単元30問（各120問）を収録。国語は漢字／語句と語彙／
  文法／古典入門、理科は植物の世界／身のまわりの物質／光・音・力／大地の変化、社会は世界の姿／
  世界の気候と暮らし／歴史のはじまり／古代国家の歩み。理科の密度・濃度・ばね・音速や、社会の時差など
  計算問題は、正答を計算式から導出して転記ミスを防いでいる（生成スクリプトは使い捨てのため非コミット）
